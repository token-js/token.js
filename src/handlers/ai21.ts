...
async function* createCompletionResponseStreaming(
  ...
  for await (const chunk of responseStream) {
    ...
    const responses: Array<AI21ChatCompletionResponseStreaming> = decodedText.split('\n')
...
export class AI21Handler extends BaseHandler<AI21Model> {
  ...
  async create(
    ...
    const axiosResponse = await axios.post("https://api.ai21.com/studio/v1/chat/completions", params, {
      headers: {
    ...
    if (body.stream === true) {
      return createCompletionResponseStreaming(axiosResponse.data, body.model, created)
    } else {
      const response: AI21ChatCompletionResponseNonStreaming = axiosResponse.data
