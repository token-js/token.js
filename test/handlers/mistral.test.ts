...
describe('findLinkedToolCallName', () => {
    ...
    const messages: (ChatCompletionMessage | ChatCompletionMessageParam)[] = [
      {
        ...
        tool_calls: [
          { id: 'tool1', function: { name: 'function1', arguments: '{}' }, type: 'function' }
        ]
      },
    ];

    const result = findLinkedToolCallName(messages as ChatCompletionMessage[], 'tool1');
  ...
  it('should error if the tool call ID is not found', () => {
    ...
    expect(() => findLinkedToolCallName(messages as ChatCompletionMessage[], 'nonexistentTool')).toThrow(InputError);
  });
});
...
describe('convertMessages', () => {
  ...
  it('should throw an error for image inputs in messages', () => {
    ...
    expect(() => convertMessages(messages)).toThrow('Image inputs are not supported by Mistral');
  });
  ...
  it('should throw an error for function messages', () => {
    ...
    expect(() => convertMessages(messages)).toThrow('Function messages are deprecated.');
  });
  ...
  it('should handle nested content types for user messages correctly', () => {
    const messages: (ChatCompletionMessageParam | ChatCompletionMessage)[] = [
      {
        role: 'user',
        content: [{ type: 'text', text: 'Text part 1' }, { type: 'text', text: 'Text part 2' }]
      }
    ];
...
describe('MistralHandler', () => {
    ...
    choices: [
      {
        ...
        message: {
          role: "assistant",
          content: "Of course, I'd be happy to share a joke with you! Here's a light-hearted one:\n\nWhy don't scientists trust atoms?\n\nBecause they make up everything!",
          tool_calls: null
        },
        finish_reason: "stop"
      }
  ...
  const mockChatStreamResponse: AsyncGenerator<ChatCompletionResponseChunk, void, unknown> = (async function* () {
    yield {
      id: "f403d3241f494981958486f3c8f16c6d",
  ...
  vi.mock('@mistralai/mistralai', (importOriginal) => ({
  ...
  const handler = new MistralHandler(handlerOptions, models.mistral.models, models.mistral.supportsJSON)
